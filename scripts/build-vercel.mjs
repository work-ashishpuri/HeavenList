 ...
 *     ESM file using esbuild (Vercel functions can't resolve node_modules
 *     from outside the function directory)
...
// ── 2. Bundle SSR server into a self-contained ESM file ──────────────────────
await build({
  entryPoints: [resolve(root, 'dist/server/server.js')],
  outfile: join(out, 'functions/index.func/bundle.mjs'),
  bundle: true,
  format: 'esm',
  platform: 'node',
  target: 'node20',
  // Vercel provides these — everything else gets inlined
  packages: 'bundle',
  // Keep dynamic imports that are computed at runtime (e.g. lazy routes)
  // but inline the static ones (assets/*.js chunks)
  splitting: false,
  allowOverwrite: true,
  logLevel: 'warning',
})
...
writeFileSync(
  join(out, 'functions/index.func/entry.mjs'),
